...
def _ppo_reward_and_cost(metrics, follower_pid_state):
    """
    Dense tracking reward plus a separate safety cost.
    The safety constraint uses THW when the follower is moving.
    """
    spacing_error = abs(float(metrics["spacing_error_m"]))
    rel_speed = abs(float(metrics["relative_velocity_mps"]))
    speed = float(metrics["ego_speed_mps"])
    distance = float(metrics["distance_m"])

    # Keep the reward simple and interpretable for the first PPO smoke test.
    reward = 1.0 - 0.05 * spacing_error - 0.02 * rel_speed
    reward = _clamp(reward, -5.0, 1.0)
    ...
    # Cost=1 means a safety violation at this step.
    thw_limit = 1.0
            ...
            try:
                ...
                while True:
                    ...
                    if reason is not None:
                        ...
                        _write_episode_csv("outputs", episode_count, rows)
                        break
