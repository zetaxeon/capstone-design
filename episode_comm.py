...
DEFAULT_SCENARIO_FILE = Path(__file__).parent / "configs" / "scenario_town03_comm_70.json"
...
def _create_comm_state(scenario_config, leader_vehicle):
    comm_config = scenario_config.get("communication", {})
    success_prob = float(comm_config.get("success_prob", 1.0))

    leader_location = leader_vehicle.get_location()
    leader_speed_mps = _get_speed_mps(leader_vehicle)

    return {
        "success_prob": success_prob,
        "last_location": _copy_location(leader_location),
        "last_speed_mps": leader_speed_mps,
        "time_since_last_success_sec": 0.0,
    }


def _observe_leader_with_comm(leader_vehicle, comm_state, dt):
    current_location = leader_vehicle.get_location()
    current_speed_mps = _get_speed_mps(leader_vehicle)

    comm_success = random.random() < comm_state["success_prob"]

    if comm_success:
        comm_state["last_location"] = _copy_location(current_location)
        comm_state["last_speed_mps"] = current_speed_mps
        comm_state["time_since_last_success_sec"] = 0.0
    else:
        comm_state["time_since_last_success_sec"] += dt

    return {
        "location": comm_state["last_location"],
        "speed_mps": comm_state["last_speed_mps"],
        "comm_success": comm_success,
        "time_since_last_success_sec": comm_state["time_since_last_success_sec"],
    }
...
def _compute_follower_control(
    ...
    # Actual values are used for evaluation/logging.
    actual_lead_speed_mps = _get_speed_mps(leader_vehicle)
    ego_speed_mps = _get_speed_mps(follower_vehicle)
    actual_distance_m = _distance_between(
        leader_vehicle.get_location(),
        follower_vehicle.get_location(),
    )

    # Observed values are what the controller receives through V2V communication.
    observed_lead_speed_mps = observed_leader_state["speed_mps"]
    observed_distance_m = _distance_between(
        observed_leader_state["location"],
        follower_vehicle.get_location(),
    )

    desired_distance_m = follower_pid_state["d0"] + follower_pid_state["h"] * ego_speed_mps

    # PID uses the observed leader state.
    control_spacing_error_m = observed_distance_m - desired_distance_m
    control_relative_velocity_mps = observed_lead_speed_mps - ego_speed_mps

    # Metrics use the actual leader/follower state.
    spacing_error_m = actual_distance_m - desired_distance_m
    relative_velocity_mps = actual_lead_speed_mps - ego_speed_mps

    follower_pid_state["integral_error"] = _clamp(
        follower_pid_state["integral_error"] + control_spacing_error_m * dt,
        -follower_pid_state["integral_limit"],
        follower_pid_state["integral_limit"],
    )

    u = (
        follower_pid_state["kp"] * control_spacing_error_m
        + follower_pid_state["ki"] * follower_pid_state["integral_error"]
        + follower_pid_state["kd"] * control_relative_velocity_mps
    )

    throttle = 0.0
    brake = 0.0
    if actual_distance_m < follower_pid_state["emergency_distance_m"]:
        brake = 1.0
    elif u >= 0.0:
    ...
    if (
        ...
        and observed_lead_speed_mps >= follower_pid_state["launch_lead_speed_threshold_mps"]
        and control_spacing_error_m >= follower_pid_state["launch_spacing_error_threshold_m"]
    ):
    ...
    thw_sec = actual_distance_m / max(ego_speed_mps, 0.1)
    ...
    metrics = {
        ...
        "lead_speed_mps": actual_lead_speed_mps,
        "ego_speed_mps": ego_speed_mps,
        "thw_sec": thw_sec,

        # Observed/control-side values.
        "observed_distance_m": observed_distance_m,
        "observed_lead_speed_mps": observed_lead_speed_mps,
        "control_spacing_error_m": control_spacing_error_m,
        "control_relative_velocity_mps": control_relative_velocity_mps,
        ...
        # Communication state.
        "comm_success": observed_leader_state["comm_success"],
        "time_since_last_comm_success_sec": observed_leader_state["time_since_last_success_sec"],
    }
...
def _write_episode_csv(output_dir, episode_count, rows):
    ...
    fieldnames = [
        ...
        "thw_sec",
        "observed_distance_m",
        "observed_lead_speed_mps",
        "control_spacing_error_m",
        "control_relative_velocity_mps",
        "throttle",
        "brake",
        "comm_success",
        "time_since_last_comm_success_sec",
    ]
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                comm_state = _create_comm_state(scenario_config, leader_vehicle)
                ...
                while True:
                    ...
                    observed_leader_state = _observe_leader_with_comm(
                        leader_vehicle,
                        comm_state,
                        dt,
                    )
                    ...
                    rows.append({
                        ...
                        "thw_sec": round(follower_metrics["thw_sec"], 3),
                        "observed_distance_m": round(follower_metrics["observed_distance_m"], 3),
                        "observed_lead_speed_mps": round(follower_metrics["observed_lead_speed_mps"], 3),
                        "control_spacing_error_m": round(follower_metrics["control_spacing_error_m"], 3),
                        "control_relative_velocity_mps": round(follower_metrics["control_relative_velocity_mps"], 3),
                        "throttle": round(follower_metrics["throttle"], 3),
                        "brake": round(follower_metrics["brake"], 3),
                        "comm_success": int(follower_metrics["comm_success"]),
                        "time_since_last_comm_success_sec": round(
                            follower_metrics["time_since_last_comm_success_sec"], 3
                        ),
                    })
                    ...
                    if reason is not None:
                        ...
                        _write_episode_csv(scenario_config.get("output_dir", "outputs_comm"), episode_count, rows)  # ← 이 줄 추가
                        break
            ...
            if args.episodes > 0 and episode_count >= args.episodes:
                logging.info("Completed %d episodes. Exiting.", episode_count)
                break
